...
class AuthController {
    ...
    public function login() {
        if (isset($_POST['submit'])) {
            $logAdmin = isset($_POST['log_admin']);
            $error = false;
            $error_email = '';
            $error_password = '';
            $email = htmlspecialchars($_POST['email']);
            ...
            if (!empty($email) && !empty($password)) {
                $this->performLogin($email, $password, $logAdmin);
            } else {
                $error = true;
                $error_email = 'Email required';
                $error_password = 'Password required';
    ...
    private function performLogin($email, $password, $logAdmin) {
        $tableName = $logAdmin ? 'admins' : 'customers';
        $role = $logAdmin ? 'admin' : 'customer';
        $checkEmailExist = $this->pdo->prepare("SELECT * FROM $tableName WHERE email = ?");
        $checkEmailExist->execute(array($email));
        $results = $checkEmailExist->fetch(PDO::FETCH_ASSOC);
        if ($results) {
            // Check if password and database password match
            if (sha1($password) == $results['password']) {
                $_SESSION["{$role}_logged"] = true;
                $_SESSION["{$role}_id"] = $results['id'];
                $_SESSION["{$role}_email"] = $results['email'];
